...
# Global variables
speriod=(15*60)-1
dbname='/home/jani/Projects/DHT11/temperaturelog.db'
...
def get_data(interval):
	...
	if interval == None:
		...
		curs.execute("SELECT * FROM " % tablename % " WHERE timestamp>datetime('now','-%s hours')" % interval)
...
def show_stats(option):
    ...
    curs.execute("SELECT timestamp,max(temp) FROM " % tablename % " WHERE timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
#    curs.execute("SELECT timestamp,max(temp) FROM temps WHERE timestamp>datetime('2017-12-01 00:00:00','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    curs.execute("SELECT timestamp,min(temp) FROM " % tablename % " WHERE timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
#    curs.execute("SELECT timestamp,min(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    curs.execute("SELECT avg(temp) FROM " % tablename % " WHERE timestamp>datetime('now','-%s hour') AND timestamp<=datetime('now')" % option)
#    curs.execute("SELECT avg(temp) FROM temps WHERE timestamp>datetime('2013-09-19 21:30:02','-%s hour') AND timestamp<=datetime('2013-09-19 21:31:02')" % option)
    ...
    rows=curs.execute("SELECT * FROM " % temperature % " WHERE timestamp>datetime('new','-1 hour') AND timestamp<=datetime('new')")
